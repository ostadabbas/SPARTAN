...
def load_from_clip(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit: bool = False, download_root: str = None, use_fft: bool = False,new_loss:bool=False):
    ...
    model = build_model(state_dict or model.state_dict(), load_from_clip = True, use_fft=use_fft, new_loss=new_loss).to(device)
