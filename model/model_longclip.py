...
class TemporalTransformer(nn.Module):
    ...
    def forward(self, x):
        # x shape: (batch_size, frame_length, input_dim)
        batch_size, frame_length, _ = x.size()
        x_fft = torch.fft.rfft(x.float(), dim=1)
        fft_features_length = x_fft.size(1)
        x_fft_features = torch.cat([x_fft.real, x_fft.imag], dim=2)
        # print(x.shape,x_fft_features.shape)
        if fft_features_length < frame_length:
            # Pad to match frame_length
            padding = frame_length - fft_features_length
            x_fft_features = torch.nn.functional.pad(x_fft_features, (0, 0, 0, padding))
        elif fft_features_length > frame_length:
            # Truncate to match frame_length
            x_fft_features = x_fft_features[:, :frame_length, :]
        x_fft_features = (x_fft_features - x_fft_features.mean()) / x_fft_features.std()
        if self.use_fft:
            x_combined = torch.cat([x, x_fft_features], dim=2)
        else:
            x_combined = x
        # Project input to hidden dimension
        x = self.input_projection(x_combined)

        # print(torch.max(x_combined), torch.min(x_combined))
        
        # Add positional encoding to maintain temporal information
        x = x + self.positional_encoding[:, :frame_length, :]
        
        # Permute for transformer input: (frame_length, batch_size, hidden_dim)
        x = x.permute(1, 0, 2)
        
        # Apply transformer encoder
        x = self.transformer_encoder(x)
        
        # Permute back: (batch_size, frame_length, hidden_dim)
        x = x.permute(1, 0, 2)
        
        # Aggregate the sequence output (e.g., using mean pooling)
        x = x.mean(dim=1)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        # Define a new linear layer to project the additional parameters
        if use_fft:
            self.temporal_attention = TemporalTransformer(input_dim=24, hidden_dim=128, num_heads=8, num_layers=4, output_dim=embed_dim)
        else:
            self.temporal_attention = TemporalTransformer(input_dim=8, hidden_dim=128, num_heads=8, num_layers=4, output_dim=embed_dim)
        # t x y z x y z w re(8) imag(8)
        self.projection = nn.Linear(2*embed_dim, embed_dim)
        self.new_short_loss = new_short_loss
    ...
    def encode_text(self, text, pose_params=None): 
        ...
        # print(x.shape) (12, 512)
        pose_params_emb = self.temporal_attention(pose_params)
        x = self.projection(torch.cat([x, pose_params_emb], dim=1))
        return x
    ...
    def forward(self, image, text_long, text_short, poses, rank):
        image = image.permute(0,2,1,3,4)
        b, t, c, h, w = image.size()
        # Remove the batch dimensions
        image = image.reshape(-1, c, h, w)
        image_features_long = self.encode_image(image)
        image_features_long = image_features_long.view(b, t, -1)  # [B, T, 512]
        # Now take the mean along the temporal direction
        image_features_long = image_features_long.mean(dim=1, keepdim=False)  # image features are now ready
        text_features_long = self.encode_text(text_long, poses)
        text_features_short = self.encode_text(text_short, poses)
        if self.new_short_loss:
            pose_params_emb = self.temporal_attention(poses)
            pose_features_short = self.PCA(pose_params_emb, 32)  
            pose_features_short = pose_features_short / pose_features_short.norm(dim=1, keepdim=True)
            pose_feature_all_short = torch.cat(torch.distributed.nn.all_gather(pose_features_short), dim=0)
        ...
        if self.new_short_loss:
            sim_i2ps = torch.matmul(image_features_short, pose_feature_all_short.T)
            sim_ps2i = torch.matmul(image_features_all_short, pose_features_short.T)
            sim_ps2i = sim_ps2i.T
            sim_i2ps = self.logit_scale.exp() * sim_i2ps
            sim_ps2i = self.logit_scale.exp() * sim_ps2i
        ...
        loss_itcl = (
                    0.5 * F.cross_entropy(sim_i2tl, targets, label_smoothing=0.1)
                    + F.cross_entropy(sim_tl2i, targets, label_smoothing=0.1)
                ) / 2
        
        if self.new_short_loss:
            loss_itcs = 0.49*F.cross_entropy(sim_i2ts, targets, label_smoothing=0.1) + 0.49*F.cross_entropy(sim_ts2i, targets, label_smoothing=0.1) + 0.01*F.cross_entropy(sim_ps2i, targets, label_smoothing=0.1) + 0.01*F.cross_entropy(sim_i2ps, targets, label_smoothing=0.1)
        else:            
            loss_itcs = (
...
def build_model(state_dict: dict, load_from_clip: bool, use_fft: bool = False, new_loss:bool=False):
    ...
    model = CLIP(
        ...
        context_length, vocab_size, transformer_width, transformer_heads, transformer_layers, load_from_clip, use_fft, new_loss
    )
    ...
    if "temporal_attention.input_projection.weight" not in state_dict:
        def init_weights(m):
            if isinstance(m, nn.Linear) or isinstance(m, nn.Parameter):
                nn.init.normal_(m.weight, std=0.02)
                if m.bias is not None:
                    nn.init.constant_(m.bias, 0)
        model.temporal_attention.apply(init_weights)

    if "projection" not in state_dict:
        nn.init.normal_(model.projection.weight, std=0.02)
